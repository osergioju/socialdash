    ...
    <AgencyLayout>
      <div style={{ padding: "32px 36px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: C.text }}>Campanhas</h1>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: C.textMuted }}>
              Organize as ações de marketing e acompanhe resultados por canal.
            </p>
          </div>
        ...
        {!loading && !error && campaigns.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            <Megaphone size={48} color={C.textDim} style={{ marginBottom: 14 }} />
            <p style={{ color: C.textMuted, fontSize: 14, margin: 0 }}>Nenhuma campanha ainda.</p>
            <p style={{ color: C.textDim, fontSize: 12, marginTop: 4 }}>Clique em "Nova Campanha" para começar.</p>
          </div>
        )}
          ...
          {campaigns.map((campaign) => {
            ...
            return (
              <div
                ...
                style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "20px 22px", cursor: "pointer", transition: "border-color 0.15s", position: "relative", borderLeft: `4px solid ${color}` }}
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                  {campaign.imageUrl ? (
                    <img src={campaign.imageUrl} alt="" style={{ width: 42, height: 42, borderRadius: 10, objectFit: "cover", background: C.border }} />
                  ) : (
                    <div style={{ width: 42, height: 42, borderRadius: 10, background: color + "25", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <Megaphone size={18} color={color} />
                    </div>
                  )}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{campaign.name}</div>
